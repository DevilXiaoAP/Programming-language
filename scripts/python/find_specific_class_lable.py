...
def file_copy(xml_file, new_xml_file, image_format):
    new_jpg_file = new_xml_file.replace('xml', image_format)
    tree = ET.parse(xml_file)
    jpg_files = xml_file.split('/')[0] + '/' + tree.find('filename').text
    try:
...
def find_classes_xml(xml_file, new_xml_file, obj_classes, find_all=1, image_format='jpg'):
    """find_all=0 仅将有标注的文件筛选到目标文件夹（清洗掉未标注图片）
       find_all=1 将标注有目标类的文件筛选到目标文件夹"""
    if find_all == 0:
        file_copy(xml_file, new_xml_file, image_format)
    if find_all == 1:
        tree = ET.parse(xml_file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', type=str, default=r'datav2/', help='image path')
    ...
    for xml_file in tqdm(xml_files):
        ...
        find_classes_xml(xml_file, new_xml, opt.obj_label, opt.find_all, opt.image_format)
